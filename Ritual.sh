...
# 사용자로부터 새로운 rpc_url과 private_key 입력받기
read -e -p "${BOLD}${CYAN}새로운 RPC URL을 입력하세요: ${NC}" rpc_url1
read -e -p "${BOLD}${GREEN}새로운 Private Key를 입력하세요: ${NC}" private_key1

# 임시 파일 생성
temp_file=$(mktemp)

# jq를 사용하여 값을 수정하고 임시 파일에 저장
jq --arg rpc "$rpc_url1" --arg priv "$private_key1" \
   '.chain.rpc_url = $rpc |
    .chain.wallet.private_key = $priv |
    .snapshot_sync.sleep = 3 |
    .snapshot_sync.batch_size = 1800 |
...
echo -e "${BOLD}${MAGENTA}config.json 파일 업데이트 완료${NC}"
...
# 사용자로부터 새로운 RPC URL과 private key 입력받기
read -e -p "${BOLD}${CYAN}새로운 RPC URL을 입력하세요: ${NC}" rpc_url1
read -e -p "${BOLD}${GREEN}새로운 Private Key를 입력하세요: ${NC}" private_key1
...
# sed를 사용하여 image 버전 변경
sed -i 's/image: ritualnetwork\/infernet-node:1.0.0/image: ritualnetwork\/infernet-node:1.2.0/' $docker_compose_file
...
read -e -p "${BOLD}${CYAN}도커가 아무것도 안 뜨나요?(y/n) : ${NC}" answer
if [[ "$answer" =~ ^[Yy]$ ]]; then
    exit 0
else
...
echo -e "${BOLD}${MAGENTA}'docker compose up'을 입력하고 숫자가 주르륵 올라오면 그 터미널을 종료하고 새로 터미널을 켜서 본인의 콘타보로 로그인하세요${NC}"
}

...
install_ritual_2() {
	...
	jq --arg img "ritualnetwork/hello-world-infernet:1.2.0" \
	   --argjson sleep 3 \
       --argjson batch_size 1800 \
		'.containers[0].image = $img |
		 .snapshot_sync.sleep = $sleep |
		 .snapshot_sync.batch_size = $batch_size' $json_1 > $temp_file
	...
	jq --arg img "ritualnetwork/hello-world-infernet:1.2.0" \
       ...
       --argjson batch_size 1800 \
		'.containers[0].image = $img |
		 .snapshot_sync.sleep = $sleep |
		 .snapshot_sync.batch_size = $batch_size' $json_2 > $temp_file
	...
	echo -e "${BOLD}${MAGENTA}Image, Sleep, Batch Size가 업데이트되었습니다.${NC}"

	echo -e "${BOLD}${MAGENTA}RPC URL과 Private Key가 업데이트되었습니다.${NC}"
